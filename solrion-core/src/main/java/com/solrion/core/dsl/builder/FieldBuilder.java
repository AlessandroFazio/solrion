...
public final class FieldBuilder {
    ...
    private static Expr.BoolExpr combine(Collection<? extends Expr.BoolExpr> parts, boolean and) {
        if (parts == null || parts.isEmpty()) return Expr.empty();
        Expr.BoolExpr acc = Expr.empty();
        for (Expr.BoolExpr p : parts) {
            if (p == null || p.isEmpty()) continue;
            acc = acc.isEmpty()
                    ? p
                    : and ? new Expr.And(acc, p) : new Expr.Or(acc, p);
        }
        return acc;
    }
    ...
    public Expr.BoolExpr in(Collection<String> values) {
        if (values == null || values.isEmpty()) return Expr.empty();
        return in(
                matchAny(
                        values.stream()
    ...
    public Expr.BoolExpr in(Expr.BoolExpr expr) {
        if (expr == null || expr.isEmpty()) return Expr.empty();
        return build(expr);
    }
    ...
    public Expr.BoolExpr boosted(String value, double boost) {
        return build(
                new Expr.Term(
                        value,
                        ValueModifiers.builder().boost(boost).build()
                )
        );
    }
