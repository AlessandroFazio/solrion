...
public class DocsResult {
    ...
    public DocsResult(
            ...
            @JsonDeserialize(contentUsing = SolrDocumentDeserializer.class)
            @JsonProperty("docs") List<SolrDocument> docs
    ) {
    ...
    public Optional<SolrDocument> last() {
        return docs == null || docs.isEmpty() ? Optional.empty() : Optional.of(docs.get(docs.size() - 1));
    }
}
