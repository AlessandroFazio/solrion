...
public final class SolrClientImpl implements SolrClient {
  ...
  public CompletionStage<SolrSelectResponse> select(SelectRequest request) {
    ...
    boolean usePostForm =
        clientOptions.alwaysPostFormForSelect()
    ...
    CompletionStage<String> sent =
        usePostForm ? transport.postForm(path, params) : transport.get(path, params);

    return sent.thenApply(this::decodeSelect)
  ...
  private String chooseCollection(String requested) {
    if (!Validate.isBlank(requested)) {
      return requested;
    }
    String collection = clientOptions.defaultCollection();
    if (!Validate.isBlank(collection)) {
      return collection;
    }
    throw new IllegalArgumentException(
        "No collection provided and no defaultCollection configured");
  }
  ...
  private void notifyCompletion(RequestStart ctx, Throwable error, long startNanos) {
    if (error == null) {
      ...
      observer.onRequestFailure(
          RequestEnd.failure(ctx, latency(startNanos), CompletionStages.unwrap(error)));
    }
  }
