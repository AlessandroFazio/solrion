...
public final class SolrClient {
    ...
    public SolrClient(
            ...
            SolrClientOptions clientOptions
    ) {
    ...
    public Multi<SolrAddBatchResult> addBatched(AddDocumentsBatchRequest request) {
        ...
        Multi<SolrAddBatchResult> batchMulti = request.docSource()
                .group()
                .intoLists()
                .of(batching.batchSize())
                .select().where(list -> list != null && !list.isEmpty())
                .onItem().transformToUni(batch ->
                        sendAddBatch(request, batch, batchIndex.getAndIncrement()))
    ...
    private Uni<SolrAddBatchResult> sendAddBatch(AddDocumentsBatchRequest request, List<SolrDocument> batch, int batchIndex) {
        Uni<SolrAddBatchResult> batchUni = add(new AddDocumentsRequest(
                request.collection(),
                request.transport(),
                batch,
                request.perBatchUpdate()))
        ...
        return batchUni.onFailure().recoverWithItem(SolrAddBatchResult.failure(batchIndex, batch.size()));
    }

    private Uni<SolrUpdateResponse> commitAtEnd(AddDocumentsBatchRequest request) {
        return commit(new CommitRequest(request.collection(), request.transport(), request.atEndUpdate()));
    }
